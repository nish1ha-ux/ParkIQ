...
# Public Subnets (For EC2 Server hosting Frontend/Backend)
...
# Private Subnets (For RDS PostgreSQL Instance)
...
# Security Group for EC2 Host (Web, REST APIs & SSH access)
...
# Security Group for RDS PostgreSQL (Accepts ingress ONLY from EC2 SG)
...
# Locate the latest Ubuntu Server AMI
data "aws_ami" "ubuntu" {
  most_recent = true
...
# EC2 instance bootstrapped with Docker & Docker Compose
resource "aws_instance" "app_server" {
  ...
  # User data script to automate installation on boot
  user_data = <<-EOF
              #!/bin/bash
              apt-get update
